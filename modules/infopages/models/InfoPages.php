...
class InfoPages extends ParentModel {
	...
	public function rules() {
		return array(
			...
			array('active, widget, widget_data, widget_position, no_show_title', 'safe'),
	...
	public function attributeLabels() {
		return array(
            ...
            'no_show_title' => tt("Не показывать заголовок", 'infopages'),
		);
	}
	...
	public static function getWidgetOptions($widget = null){
		...
		if (issetModule('seo')) {
			$arrWidgets = CMap::mergeArray($arrWidgets, array('seosummaryinfo' => tc('Summary_infopage_listing')));
		}
	...
	public function beforeSave(){
        if(($this->widget == 'apartments' || $this->widget == 'seosummaryinfo') && isset($_POST['filter'])){
	...
	public function getCriteriaForAdList(){
		...
		if ($result) {
			...
			if ($result && !empty($this->seasonalPricesIds)) {
				$ids = array_intersect($result, $this->seasonalPricesIds);
				$criteria = new CDbCriteria;
