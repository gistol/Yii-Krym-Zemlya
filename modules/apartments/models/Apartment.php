...
class Apartment extends ParentModel {
    ...
    public function relations() {
        ...
        if (issetModule('seasonalprices')) {
            if (isset(Yii::app()->controller) && isset(Yii::app()->controller->apType) && Yii::app()->controller->apType) {
                ...
                $relations['seasonalPrices_sort_desc'] = array(self::HAS_ONE, 'Seasonalprices', 'apartment_id', 'order' => 'seasonalPrices_sort_desc.price DESC', 'condition' => 'seasonalPrices.price_type = ' . Yii::app()->controller->apType);
                $relations['seasonalPrices_sort_asc'] = array(self::HAS_ONE, 'Seasonalprices', 'apartment_id', 'order' => 'seasonalPrices_sort_asc.price ASC', 'condition' => 'seasonalPrices.price_type = ' . Yii::app()->controller->apType);
            } else {
    ...
    public function search() {
        ...
        if ($this->ownerUsername) {
            $criteria->addCondition('user.username LIKE "%' . $this->ownerUsername . '%"');
        }
    ...
    public function makeClone() {
        ...
        if (newFolder($newPathUploads)) {
            ...
            if (newFolder($newPathImages) && newFolder($newPathImagesModified)) {
                ...
                if ($result) {
                    foreach ($result as $res) {
                        $sql = 'INSERT INTO {{images}} (id_object, id_owner, file_name, sorter, is_main, date_created)
                            VALUES ("' . $this->id . '", "' . $res['id_owner'] . '", "' . $res['file_name'] . '", "' . $res['sorter'] . '", "' . $res['is_main'] . '", NOW())';
                        Yii::app()->db->createCommand($sql)->execute();
    ...
    public static function returnMainThumbForGrid($data = null, $addClass = '') {
        if ($data) {
